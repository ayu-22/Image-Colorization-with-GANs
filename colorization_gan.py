...
x16 = Conv2D(3, (3,3), activation = 'tanh', padding='same', strides=1)(x15)

generator = Model(inputs = generator_inp, outputs = x16)
...
gans.compile(loss=["mse", "binary_crossentropy"], loss_weights=[100., 1.], optimizer=adam)
...
for epoch in range(100):
    ...
    for i in range(itera):
      ...
      real = np.ones(color.shape[0]) - 0.1
      fake = np.zeros(lab_color_img.shape[0])*0.1
    ...
    losg1.append(gan_mean)
    losd1.append(dis_mean)
    K.set_value(discriminator.optimizer.lr, lr_schedule(epoch))
    K.set_value(gans.optimizer.lr, lr_schedule(epoch))
